...
class PageController {
    ...
    public String api(Model model) {
        try {
            Map response = keycloakRestClient.get().uri("http://localhost:18090/api/greetings/me").retrieve().body(Map.class);
    ...
    public String adminApi(Model model) {
        try {
            Map response = keycloakRestClient.get().uri("http://localhost:18090/api/admin").retrieve().body(Map.class);
            model.addAttribute("apiResponse", response);
        } catch (Exception ex) {
            model.addAttribute("apiResponse", Map.of("error", ex.getMessage()));
        }
